...
const Header = (props) => {
  ...
  return (
    <Flex w="full" align="center" justify="space-between" px="110px" pt="67px">
      <Heading as={Link} to="/" color="darkGray" fontSize="xl">avocado</Heading>
      <Flex>
        <HStack color="darkGray" spacing="7" fontSize="xs">
          {menus && menus.map((menu, i) => {
            const isActive = match.path === `/${menu.text}` || (match.path === '/' && menu.text === 'home');
            const activeStyle = { borderBottomWidth: '2px', borderBottomColor: 'green', color: 'green', fontWeight: 'bold' };
            const defaultStyle = { borderBottomWidth: '0', borderBottomColor: 'darkGray', color: 'darkGray', fontWeight: '500' };
            const style = isActive ? activeStyle : defaultStyle;
            return (<Box key={i} py="3px" {...style}><Link to={menu.href}>{menu.text}</Link></Box>);
          })}
        </HStack>
